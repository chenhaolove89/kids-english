...
- **学英语**：1998 单词 / 53 分类（动物、野生动物、家人、食物、国家国旗、太空、职业、常用词、词族拼读……），看图学词 + 听音选图挑战
...
> 古诗已覆盖全部四学段（启蒙 12 首、其余三学段各 6 首）。**176 门课程全部 `available`**——2026-09-17 的 draft 复核
> 把最后三门课转正：`math-practice-l6`（小数与分数）、`math-practice-l9`（四则混合）、`en-quiz-l4`（进阶词汇挑战），
> 现在数学 9 关全开放。发布状态机制（`lessonStatus` 支持 `draft`，draft 不进地图/旧入口/继续学习/家长统计）保留可用，
> 只是当前没有 draft 课。

## 路线图

- ✅ Phase 0/1：架构分层 + 课程目录（**176 课**）+ 课时会话与事件流 + 首答星级 + 继续学习 + 本机学习摘要
...
- ⬜ Phase 2 未完：五六年级阅读/写作内容（数学高年级关卡已于 2026-09-17 开放：小数与分数、四则混合）
