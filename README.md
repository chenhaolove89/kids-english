...
- **学语文**：297 个常用汉字分级识字（字 + 拼音 + 例词 + 真人有声）+ 每字一句小短句 + 1420 词语卡 + 必背古诗点读 + 笔顺描红 + **阅读理解短文**（一二/三四年级各 5 篇原创短文，整篇朗读 + 3 题作答），听音识字挑战
...
tools/smoke-h5.mjs        真实 Chrome + CDP 冒烟测试（零依赖）：186 项断言，页面渲染/交互/作答/会话恢复/存储/零报错（npm run smoke:h5）
...
src/pages/reading/        语文阅读理解：短文书单 / 读短文（整篇朗读）/ 3 题作答（首答星级）
...
npm run test                 # Node 测试（348 个：出题/判题/星级/会话/存储迁移/目录/错题本/图鉴/偏好核心/分层门禁/源码契约）
npm run smoke:h5             # 真实 Chrome 冒烟测试 186 项（约 8~10 分钟；需先 build:h5 并另开终端跑 node tools/serve.mjs）
...
> 古诗已覆盖全部四学段（启蒙 12 首、其余三学段各 6 首）。**语文阅读理解短文**已接入一二/三四年级
> （各 5 篇原创短文，每篇整篇朗读 + 3 道选择题；作答按 challenge 计课时、按首答正确率给星，图鉴「短文图鉴」一格一篇）。
> 数学 2026-09-17 从 9 关补铺到 **16 关**（新增形状识别、
> 认识时间、长度与测量、周长与面积、分数初步、百分数与比例、统计与数据），四学段分别是 3/4/5/4 关，
> 新关卡复用现成提示音、**零新增音频**。**191 门课程全部 `available`**——2026-09-17 的 draft 复核把最后三门课转正：
...
- ✅ Phase 0/1：架构分层 + 课程目录（现 **191 课**）+ 课时会话与事件流 + 首答星级 + 继续学习 + 本机学习摘要
...
- 🚧 Phase 2 进行中：**英语句子阶梯**已铺完四层（启蒙日常用语 / 一二小短句 / 三四稍长句 / 五六长句，各 20 句，`tools/en-sentences.csv` 为唯一数据源）；**语文篇章阅读**已接入一二/三四年级（各 5 篇，`content-packages/zh-passages.json` 为唯一真源）；仍缺英语篇章阅读与写作材料（长句不等于篇章阅读）
...
- `learn`（学一学）/ `challenge`（挑战）**计入课时完成**；挑战另按首答正确率给星。
  阅读理解短文也是这条口径：课卡在目录里是「学一学」卡（要出现在课程地图与解锁路径上），
  但页面的作答记 `challenge` 会话——一篇文章一次挑战，3 题答完算完成、按首答给星。
...
- 阅读理解**按篇点亮**图鉴「短文图鉴」：做完整篇的挑战 → 认识，3 题首答全对 → 读懂（见 `src/domain/passage.js`）。
